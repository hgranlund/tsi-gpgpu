 ...
 */
float matrixMultiply(int argc, char **argv, int block_size, dim3 &dimsA, dim3 &dimsB)
{
    ...
    const float valB = M_PI;
    constantInit(h_A, size_A, valB);
    ...
    {
        double abs_err = fabs(h_C[i] - (dimsA.x * valB*valB));
