...
\subsubsection{From a recursive to a iterative implementation} % (fold)
...
% subsubsection divide_work_on_cuda (end)



% TODO: Header must be improved
\subsubsection{Parallelizing balance node} % (fold)
...
\begin{algorithmic}
    ...
    \Function{Balance-Subtree}{$T$, $d$}
        ...
        \Repeat
            \ForAll {$n \in P$}
                \State $Z(t) \gets $  \text{Occurrences of zeros in radix, $r$, found by thread, $t$}.
            \EndFor
            \State $c \gets \Call{Sum-Reduce}{Z}$ \Comment{$c$ is the cut if the current partition $p$}
            \If{$u-c >= n/2$}
                \State$ u \gets u-c$
